...
- *Pricing model:* (5+5+1+5+5+5) ÷ 6 = **4.3**
...
|--------|--------|-----------|
| Geographic / institutional reach | **Eliminate** | We intentionally serve one bus route at one university; broad route coverage is what incumbents compete on, but it is irrelevant to our target user and we have eliminated it entirely. |
...
| Switching cost / user lock-in | **Raise** | Trust built through consistent honesty creates genuine lock-in. Three weeks of personal history and schedule data cannot be replicated by any alternative. |
...
| Pre-departure usefulness | **Raise** | Widest gap (we score 5, industry avg 2.0). Pre-departure info is the #1 documented pain point; we raise it far above the industry standard. |
| Honest status labelling (CONFIRMED vs ESTIMATED) | **Create** | No competitor distinguishes between confirmed and estimated bus status. This is a genuinely new dimension in this market. *(Note: badge currently defaults to CONFIRMED; ESTIMATED-default gating on data freshness is committed for Sprint 2 — see `risk-register.md` R2.)* |
...
Our Blue Ocean move is to exit that competition entirely and shift the value proposition to the moment before departure. We stopped competing on geographic breadth — we cover one route, deliberately — and we stopped competing in the category of "real-time tracking once you're already at the stop." Instead, we raised pre-departure decision support to a level no incumbent provides, because it is useful only to a niche they do not recognise as their customer.

The combination of actions we took makes this configuration defensible. We raised segment fit and personalisation to address KIU Bus #3 commuters specifically, which means every feature is built around the intercity student's highest-stakes scenario: a missed marshrutka connection costs a half-day, not just 30 minutes. We raised pre-departure usefulness far above the industry average, giving students an accurate answer to "should I leave now?" before they have incurred any waiting cost. We reduced geographic reach and feature scope to keep the product fast and trustworthy within its narrow domain, rather than adequate across a wide one. We also created a dimension the industry lacks entirely: honest CONFIRMED/ESTIMATED status labelling, which gives students transparency about whether the arrival time is verified or estimated.
