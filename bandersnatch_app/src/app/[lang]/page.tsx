    ...
    <main className="flex-grow flex flex-col items-center justify-center p-5 pb-32">
      ...
      >
        ...
        <motion.div layout="position" className="mb-6">
          <h1 className="text-3xl font-bold text-on-surface dark:text-slate-100 mb-2 tracking-tight">{dict.title}</h1>
        ...
        <motion.div layout="position" className="mb-6 z-20 relative">
          <label className="block font-bold text-on-surface dark:text-slate-200 mb-1 text-sm tracking-wide" htmlFor="stop-selector">
            {dict.currentStop}
          </label>
        ...
        <AnimatePresence mode="popLayout">
          ...
          {status === "loading" && (
            ...
            >
              ...
              <p className="text-on-surface dark:text-slate-200 font-medium animate-pulse">{loadingMessage}</p>
            </motion.div>
          )}
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h2 className="text-4xl font-black text-on-surface dark:text-slate-100 tracking-tighter">7 <span className="text-lg font-medium text-on-surface-variant dark:text-slate-400">{dict.mins}</span></h2>
                  <p className="text-sm text-on-surface-variant dark:text-slate-400 mt-1">{dict.arrivingSoon}</p>
                </div>
                <div className="flex flex-col items-end text-right">
                  <div className="text-xs font-bold text-on-surface-variant dark:text-slate-400 uppercase tracking-wider mb-1">{dict.crowding}</div>
                  <div className={`flex items-center gap-1 ${
                    crowding === "Low" ? "text-success dark:text-green-400" : crowding === "Medium" ? "text-warning dark:text-yellow-400" : "text-red-600 dark:text-red-400"
                  }`}>
      ...
      >
        ...
        <motion.div whileTap={{ scale: 0.95 }} className="bg-surface-container dark:bg-slate-900 border border-outline-variant dark:border-slate-800 rounded-lg p-3 flex flex-col justify-between h-24 shadow-sm cursor-pointer hover:shadow-md transition-all">
          ...
          <div className="text-2xl font-black text-on-surface dark:text-slate-200">{dict.none}</div>
        </motion.div>
      </motion.div>
    </main>
  );
